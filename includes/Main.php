...
final class Main
{
     ...
     * @var string
     */
    public const PREFIX = 'wp_user_statistics';
    ...
    public static function uninstall_plugin()
    {
        ...
        (new \WPUserStatistics\Migrations())->cleanUp(self::PREFIX, $settings);
    }
    ...
    public function run()
    {
        ...
        if (defined('WP_CLI') && \WP_CLI) {
            $this->container['cli'] = new \WPUserStatistics\CliLoader(self::PREFIX);
        }
    ...
    {
        (new \WPUserStatistics\Migrations())->run(self::PREFIX, $this->VERSION);
    ...
    {
        // initialize assets
        $this->container['assets'] = new \WPUserStatistics\Assets(self::PREFIX);

        // initialize the various loader classes
        if ($this->is_request('admin')) {
            $ctx = new \WPUserStatistics\AdminLoader(self::PREFIX);
            $this->container['admin'] = $ctx;
        }

        if ($this->is_request('frontend')) {
            $this->container['frontend'] = new \WPUserStatistics\FrontendLoader(self::PREFIX);
        }

        if ($this->is_request('ajax')) {
            // $this->container['ajax'] =  new \WPUserStatistics\AjaxLoader(self::PREFIX);
        }

        // finally load api routes
        $this->container['api'] = new \WPUserStatistics\ApiRoutes(self::PREFIX);
    }
